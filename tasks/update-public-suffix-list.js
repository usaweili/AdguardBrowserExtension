...
const convertListToObject = (list) => {
    ...
    const suffixesWithoutCommentsAndSpaces = rows.filter(row => !(row.length === 0 || row.indexOf('//') !== -1));
    const suffixesWithoutSpecialRules = suffixesWithoutCommentsAndSpaces.filter(suffix => !(suffix.indexOf('*') !== -1 || suffix.indexOf('!') !== -1));
    const suffixesInPunycode = suffixesWithoutSpecialRules.map(suffix => punycode.toASCII(suffix));
...
const updateSuffixes = () => request(PUBLIC_SUFFIXES_URL, async (error, response, body) => {
